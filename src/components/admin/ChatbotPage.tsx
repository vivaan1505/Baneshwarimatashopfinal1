...
import { Plus, Edit, Trash, X, Check, MessageSquare } from 'lucide-react';
...
const ChatbotPage: React.FC = () => {
  ...
  const { register: registerScript, handleSubmit: handleSubmitScript, reset: resetScript, watch: watchScript, formState: { errors: scriptErrors } } = useForm<{
    name: string;
    section: string;
    trigger_keywords: string;
    response: string;
  ...
  const filteredScripts = scripts.filter(script => {
    ...
    const matchesSection = sectionFilter === 'all' || script.section === sectionFilter;
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow-sm p-6 dark:bg-gray-800">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-medium dark:text-white">Chatbot Settings</h2>
              {!editingSettings ? (
                <button
                ...
                {editingSettings ? (
                  <form onSubmit={handleSubmitSettings(onSubmitSettings)} className="space-y-4">
                    <div>
                      ...
                      {settingsErrors.welcome_message && (
                        <p className="mt-1 text-sm text-red-600 dark:text-red-400">{settingsErrors.welcome_message.message}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      {settingsErrors.fallback_message && (
                        <p className="mt-1 text-sm text-red-600 dark:text-red-400">{settingsErrors.fallback_message.message}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      {settingsErrors.auto_response_delay && (
                        <p className="mt-1 text-sm text-red-600 dark:text-red-400">{settingsErrors.auto_response_delay.message}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      {settingsErrors.human_handoff_threshold && (
                        <p className="mt-1 text-sm text-red-600 dark:text-red-400">{settingsErrors.human_handoff_threshold.message}</p>
                      )}
                    </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className={`mt-1 ${settings.is_enabled ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                        {settings.is_enabled ? 'Enabled' : 'Disabled'}
                      </p>
                    </div>
                    <div>
                      <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Welcome Message</h3>
                      <p className="mt-1 text-gray-600 dark:text-gray-400">{settings.welcome_message}</p>
                    </div>
                    <div>
                      <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Fallback Message</h3>
                      <p className="mt-1 text-gray-600 dark:text-gray-400">{settings.fallback_message}</p>
                    </div>
                    <div>
                      <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Auto-Response Delay</h3>
                      <p className="mt-1 text-gray-600 dark:text-gray-400">{settings.auto_response_delay} ms</p>
                    </div>
                    <div>
                      <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Human Handoff Threshold</h3>
                      <p className="mt-1 text-gray-600 dark:text-gray-400">{settings.human_handoff_threshold} failed responses</p>
                    </div>
                    <div>
                      <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Last Updated</h3>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mt-6 dark:bg-gray-800">
            ...
            <form onSubmit={handleSubmitScript(onSubmitScript)} className="space-y-4">
              <div>
                ...
                />
                {scriptErrors.name && (
                  <p className="mt-1 text-sm text-red-600 dark:text-red-400">{scriptErrors.name.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                {scriptErrors.section && (
                  <p className="mt-1 text-sm text-red-600 dark:text-red-400">{scriptErrors.section.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                {scriptErrors.trigger_keywords && (
                  <p className="mt-1 text-sm text-red-600 dark:text-red-400">{scriptErrors.trigger_keywords.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                {scriptErrors.response && (
                  <p className="mt-1 text-sm text-red-600 dark:text-red-400">{scriptErrors.response.message}</p>
                )}
              </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-sm overflow-hidden dark:bg-gray-800">
            <div className="p-6 border-b dark:border-gray-700 flex justify-between items-center">
              <h2 className="text-lg font-medium dark:text-white">Chatbot Scripts</h2>
              <div className="flex items-center">
                <MessageSquare className="text-primary-600 mr-2 dark:text-primary-400" />
                <span className="text-sm font-medium dark:text-gray-300">{scripts.length} Scripts</span>
              </div>
            </div>
            ...
            <div className="p-4 border-b dark:border-gray-700">
              <div className="flex flex-wrap gap-4">
                ...
                <select
                  ...
                  className="border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                >
            ...
            {loading ? (
              ...
              <div>
                {sectionFilter === 'all' ? (
                  ...
                  sections.map(section => {
                    const sectionScripts = filteredScripts.filter(script => script.section === section);
                    if (sectionScripts.length === 0) return null;
                    
                    return (
                      <div key={section} className="border-b last:border-b-0 dark:border-gray-700">
                        <div className="px-6 py-3 bg-gray-50 dark:bg-gray-700">
                          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">{section}</h3>
                        </div>
                        <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                          {sectionScripts.map((script) => (
                            <li key={script.id} className="p-6 hover:bg-gray-50 dark:hover:bg-gray-700">
                              <div className="flex justify-between items-start">
                                <div>
                                  <div className="flex items-center">
                                    <h3 className="text-lg font-medium text-gray-900 dark:text-white">{script.name}</h3>
                                    {!script.is_active && (
                                      <span className="ml-2 px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800 dark:bg-gray-600 dark:text-gray-300">
                                        Inactive
                                      </span>
                                    )}
                                  </div>
                  ...
                  // Show flat list when filtered by section
                  <ul className="divide-y divide-gray-200 dark:divide-gray-700">
