...
-- Add storage policies for brand logos
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.policies 
    WHERE name = 'Brand logos are publicly accessible'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.policies 
    WHERE name = 'Brand logos are uploadable by admins'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.policies 
    WHERE name = 'Brand logos are deletable by admins'
  ) THEN
...
-- Add storage policies for coupon images
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.policies 
    WHERE name = 'Coupon images are publicly accessible'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.policies 
    WHERE name = 'Coupon images are uploadable by admins'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.policies 
    WHERE name = 'Coupon images are deletable by admins'
  ) THEN
